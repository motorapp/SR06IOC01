...
epicsEnvSet("P", "SR06DMC01:")

##################################################################################################

#Load motor records for real and coordinate system (CS) motors
#Motor record version 6-10 and up
dbLoadTemplate("$(TOP)/SR06IOC01App/Db/$(PORT)_motors-v6-10up.substitutions", "P=$(P), PORT=$(PORT)")

#Load DMC controller features (eg.  Limit switch type, home switch type, output compare, message consoles)
dbLoadTemplate("$(TOP)/SR06IOC01App/Db/galil_dmc_ctrl.substitutions", "P=$(P), PORT=$(PORT)")

#Load Amplifier status monitors (eg. Overtemperature, OverVoltage, ELO, etc)
dbLoadTemplate("$(TOP)/SR06IOC01App/Db/galil_quadAmpStatus.substitutions", "P=$(P), PORT=$(PORT)")

#Load extra features for real axis/motors (eg. Motor type, encoder type)
dbLoadTemplate("$(TOP)/SR06IOC01App/Db/$(PORT)_motor_extras.substitutions", "P=$(P), PORT=$(PORT)")

#Load extra features for CS axis/motors (eg. Setpoint monitor)
dbLoadTemplate("$(TOP)/SR06IOC01App/Db/galil_csmotor_extras.substitutions", "P=$(P), PORT=$(PORT)")

#Load kinematics for CS axis/motors (eg. Forward and reverse kinematics, kinematic variables)
dbLoadTemplate("$(TOP)/SR06IOC01App/Db/galil_csmotor_kinematics.substitutions", "P=$(P), PORT=$(PORT)")

#Load coordinate system features (eg. Coordinate system S and T status, motor list, segments processed, moving status)
dbLoadTemplate("$(TOP)/SR06IOC01App/Db/galil_coordinate_systems.substitutions", "P=$(P), PORT=$(PORT)")

#Load digital IO databases
dbLoadTemplate("$(TOP)/SR06IOC01App/Db/galil_dmc_digital_ports.substitutions", "P=$(P), PORT=$(PORT)")

#Load analog IO databases
dbLoadTemplate("$(TOP)/SR06IOC01App/Db/galil_dmc_analog_ports.substitutions", "P=$(P), PORT=$(PORT)")

#Load user defined functions
dbLoadTemplate("$(TOP)/SR06IOC01App/Db/galil_userdef_records.substitutions", "P=$(P), PORT=$(PORT)")

#Load user defined array support
dbLoadTemplate("$(TOP)/SR06IOC01App/Db/galil_user_array.substitutions", "P=$(P), PORT=$(PORT)")

#Load profiles
dbLoadTemplate("$(TOP)/SR06IOC01App/Db/$(PORT)_profileMoveController.substitutions", "P=$(P), PORT=$(PORT)")
dbLoadTemplate("$(TOP)/SR06IOC01App/Db/$(PORT)_profileMoveAxis.substitutions", "P=$(P), PORT=$(PORT)")
...
set_pass0_restoreFile("SR06IOC01_$(PORT)Settings.sav")
# restore positions in pass 0 so motors don't move
set_pass0_restoreFile("SR06IOC01_$(PORT)Positions.sav")
# restore kinematic equation character arrays in pass 1
set_pass1_restoreFile("SR06IOC01_$(PORT)Kinematics.sav")
